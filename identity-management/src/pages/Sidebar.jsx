...
const Sidebar = () => {
  ...
  return (
    <div
      className={`transition-all duration-300 ease-in-out bg-[#3c3836] border-r border-[#504945] min-h-screen p-4 flex flex-col shadow-sm ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      <div className="flex items-center justify-between mb-6">
      ...
      <nav className="flex-1">
        {navItems.map((item) => (
          <Link
            key={item.label}
            to={item.path}
            className="flex items-center gap-3 p-3 text-sm text-[#ebdbb2] rounded-lg hover:bg-[#504945] transition-all"
          >
            <span className="w-5 h-5">{item.icon}</span>
            <span
              className={`transition-all duration-300 ${
      ...
      {!collapsed && (
        <div className="mt-auto">
          <h4 className="text-xs text-[#a89984] mb-2 px-2">Projects</h4>
          <div className="px-2 space-y-2">
            <div>
              <p className="text-sm text-[#ebdbb2]">Update Text Styles</p>
              <div className="w-full bg-[#504945] rounded-full h-1.5">
                <div className="bg-blue-500 h-1.5 rounded-full w-1/4"></div>
              </div>
            </div>
            <div>
              <p className="text-sm text-[#ebdbb2]">Add Components</p>
              <div className="w-full bg-[#504945] rounded-full h-1.5">
                <div className="bg-yellow-500 h-1.5 rounded-full w-3/4"></div>
              </div>
            </div>
            <div>
              <p className="text-sm text-[#ebdbb2]">Checkboxes Checked</p>
              <div className="w-full bg-[#504945] rounded-full h-1.5">
                <div className="bg-green-500 h-1.5 rounded-full w-1/2"></div>
              </div>
            </div>
